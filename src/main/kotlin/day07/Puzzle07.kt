        ...
        if (it.startsWith("$ cd")) {
            when (val toDir = it.split(" ")[2]) {
                ...
                ".." -> current = current.parent!!
                    ...
                    var next = current.children.firstOrNull { n -> !n.isFile && n.id == toDir }
                    if (next == null) {
                        next = Node(toDir)
                        next.parent = current
            ...
            var dirNode = current.children.firstOrNull { n -> !n.isFile && n.id == dirName }
            if (dirNode == null) {
            ...
            var fileNode = current.children.firstOrNull { n -> n.isFile && n.id == filename }
            if (fileNode == null) {
                fileNode = Node(filename, true)
                fileNode.parent = current
                fileNode.size = parts[0].toInt()
...
class Node(var id: String, var isFile: Boolean = false) {
    var parent: Node? = null
    var size: Int = 0
    val children = mutableSetOf<Node>()

    fun getSizeWithChildren(): Int {
        if (isFile) return size

        val stack = ArrayDeque(children)
        var s = 0
        while (stack.isNotEmpty()) {
            s += stack.removeFirst().getSizeWithChildren()
        }
        return s
    }

    override fun toString(): String {
        return "Node(id='$id', isFile=$isFile, parent='${parent?.id}', size=$size, children=${children.map { it.id }})"
    }
    ...
    override fun hashCode(): Int {
        return id.hashCode() + parent?.id.hashCode()
    }
}
